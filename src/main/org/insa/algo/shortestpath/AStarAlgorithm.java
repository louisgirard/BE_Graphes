...
public class AStarAlgorithm extends DijkstraAlgorithm {
    ...
    protected ShortestPathSolution doRun() {
        ...
        LabelStar labelCourant = new LabelStar();
        LabelStar labelFils = new LabelStar();
        
        //initialisation
        for (int i = 0; i < data.getGraph().size(); i++) {
        	tabLabel.add(new LabelStar(data.getGraph().get(i),data));
        }
        ...
        while(!labelHeap.isEmpty() && !labelCourant.getCurrentNode().equals(data.getDestination())) {
        	nbIterations++;
        	labelCourant = (LabelStar)labelHeap.deleteMin();
